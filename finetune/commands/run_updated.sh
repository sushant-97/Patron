...
train_label=$2; # number of labels 32/64/128
if_method=$3;
trial=$4
...
output_dir=${task}/${trial}/${if_method}/${train_label}/model
mkdir -p ${output_dir}
echo ${method}
mkdir -p ${task}/${trial}/${if_method}/${train_label}/cache
...
train_cmd="CUDA_VISIBLE_DEVICES=${gpu} python3 main.py --do_train --do_eval --task=${task} \
	...
	--data_dir="../${task}/${trial}/${if_method}" --train_seed=${train_seed} \
	--cache_dir="${task}/${trial}/${if_method}/${train_label}/cache" \
	...
	--sample_labels=${train_label} --al_method=${al_method} --if_method=${if_method} --trial_=${trial}"
